...
export function AdminKahWorkforceBoard() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const res = await fetch("/api/admin/kah-workforce-prospects");
  ...
  async function updateStatus(id: string, status: ProspectStatus) {
    setUpdating(id);
    await fetch("/api/admin/kah-workforce-prospects", {
      method: "PATCH",
  ...
  async function fireBatch() {
    ...
    for (let i = 0; i < toSend.length; i++) {
      ...
      try {
        const res = await fetch("/api/admin/kah-workforce-send-one", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <div className="border-b border-white/10 bg-gray-900 px-6 py-4">
        <div className="mx-auto max-w-7xl flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400">Freelances / créateurs / indépendants · Resend · contact@kah-digital.ch → kahdigital42@gmail.com</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <FiZap size={13} /> {firing ? "Envoi..." : `Lancer batch (10 emails)`}
            </button>
            <button
              onClick={() => void load()}
              disabled={loading}
        ...
        <div className="mx-auto max-w-7xl px-6 py-6 space-y-6">
          ...
          <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 text-xs text-gray-400">
              ...
              <div className="flex gap-2"><span>✅</span><span><strong className="text-white">Gagné</strong> — compte Pro actif sur kah-workforce</span></div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <StatCard label="Envoyés" value={stats.sent} sub="freelances / créateurs" />
            <StatCard
              label="Taux réponse"
          ...
          <div className="rounded-2xl border border-violet-500/30 bg-violet-500/5 p-6">
            <h3 className="mb-1 font-bold text-white flex items-center gap-2 text-lg">
              <FiZap size={18} className="text-violet-400" /> Prospection automatique
            </h3>
            <p className="text-sm text-gray-400 mb-5">
              Découverte de leads tous les jours à 8h30 (lun–ven) · séquence J0/J3/J7 à 13h30 (lun–ven), <strong className="text-white">5 emails/run</strong>.
              {stats.pending > 0 && (
                <span className="ml-2 text-gray-300">
                  <strong className="text-violet-300">{stats.pending} prospects</strong> en attente.
                </span>
              )}
            </p>
                ...
                return (
                  <div key={p.id} className={`rounded-2xl border p-5 ${p.status === "demo_scheduled" ? "border-violet-500/40 bg-violet-500/5" : "border-emerald-500/30 bg-emerald-500/5"}`}>
                    ...
                    <div className="flex gap-2">
                      <a
                        href={`https://mail.google.com/mail/?view=cm&to=${encodeURIComponent(p.email)}&su=${encodeURIComponent(`Suite — ${p.company} · KAH Workforce`)}`}
                        target="_blank"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/8 text-left text-xs text-white/40 uppercase tracking-wider">
                    ...
                    <th className="px-4 py-3 hidden md:table-cell">Profil</th>
