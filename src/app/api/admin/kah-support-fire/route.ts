...
function buildEmail(p: Prospect): { subject: string; html: string } {
...
<div style="max-width:600px;margin:40px auto;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08);">
  ...
  <div style="padding:32px 36px;">
    ...
    <div style="text-align:center;margin-bottom:26px;">
      <a href="https://kah-support.ch/glpi" style="display:inline-block;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;">
  ...
  <div style="padding:18px 36px;background:#f9fafb;border-top:1px solid #e5e7eb;">
    <p style="color:#6b7280;font-size:13px;margin:0;line-height:1.6;"><strong>PS :</strong> Si vous voulez voir ça sur un GLPI réel, je peux vous faire une démo de 10 min. Répondez simplement à cet email.</p>
  </div>
...
export async function POST(): Promise<Response> {
  ...
  if (prospects.length === 0) {
    return NextResponse.json({ type: "done", sent: 0, failed: 0, message: "Aucun prospect en attente" });
  }
      ...
      for (let i = 0; i < prospects.length; i++) {
        ...
        try {
          ...
          await supabase
            ...
            .eq("id", p.id);
          sent++;
          send({ type: "progress", step: i + 1, total: prospects.length, email: p.email, company: p.company, ok: true });
        } catch (err) {
          failed++;
          send({ type: "progress", step: i + 1, total: prospects.length, email: p.email, company: p.company, ok: false, error: String(err) });
        }
      }

      send({ type: "done", sent, failed, total: prospects.length });
      controller.close();
    },
  });
