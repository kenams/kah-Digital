    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="mx-auto max-w-2xl px-4 py-8">
        ...
        <div className="mb-6 text-center">
          <h1 className="mb-2 text-2xl font-extrabold tracking-tight">{c.greeting(name)}</h1>
          <p className="text-gray-400 text-sm">{c.subtitle}</p>
        </div>

        {/* ── CTA EN HAUT — visible sans scroll ── */}
        <div className="mb-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/8 p-5">
          <p className="mb-1 text-center text-base font-black text-white">
            {lang === "en" ? "Get a free quote in 24h" : lang === "de" ? "Kostenloses Angebot in 24h" : "Devis gratuit sous 24h"}
          </p>
          <p className="mb-4 text-center text-xs text-gray-400">
            {lang === "en" ? "No commitment — based on your analysis" : lang === "de" ? "Unverbindlich — basierend auf Ihrer Analyse" : "Sans engagement — basé sur votre analyse"}
          </p>
          <a
            href={waUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mb-3 flex items-center justify-center gap-2 rounded-full bg-[#25D366] px-6 py-3.5 font-bold text-white shadow-lg transition hover:bg-[#22c55e]"
          >
            ...
            {lang === "en" ? "Reply on WhatsApp — fast answer" : lang === "de" ? "Auf WhatsApp antworten" : "Répondre sur WhatsApp — réponse rapide"}
          </a>
          <QuickContactForm prospectId={prospect.id} siteUrl={prospect.website} businessName={prospect.businessName ?? ""} lang={lang as "en" | "fr" | "de"} />
        </div>
